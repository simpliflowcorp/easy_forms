...
- **Health monitoring** (`src/lib/llmHealthMonitor.ts`): a global `setInterval` every 10s pings `https://integrate.api.nvidia.com/v1/models`, dedupes status-changes, and `pubSub.publish("agent:llm_health", {status})`. Also honours `agent:simulated_offline:{ticketId}` to force "offline" for one ticket under test.
